...
# Goal 5.1 Implementation - Security and Access Control System (Simple Final)
# Priority: High
...
create_user() {
    ...
    # Add user to database (simple format: username:hash:role:failed_attempts)
    echo "${username}:${hashed_password}:${role}:0" >> "$USERS_DB"
...
authenticate_user() {
    ...
    # Parse user data (simple format)
    IFS=':' read -r stored_username stored_hash stored_role failed_attempts <<< "$user_line"
    ...
    if [[ "$input_hash" == "$stored_hash_only" ]]; then
        # Successful login - update failed attempts to 0
        update_user_failed_attempts "$username" 0
        ...
        update_user_failed_attempts "$username" "$new_failed_attempts"
...
# Update user failed attempts (simple approach)
update_user_failed_attempts() {
    local username="$1"
    local new_failed_attempts="$2"
    
    # Create temporary file
    local temp_file=$(mktemp)
    
    # Copy all lines except the target user
    grep -v "^${username}:" "$USERS_DB" > "$temp_file" 2>/dev/null || true
    
    # Get the original user line
    local user_line=$(grep "^${username}:" "$USERS_DB" 2>/dev/null || echo "")
    if [[ -n "$user_line" ]]; then
        IFS=':' read -r stored_username stored_hash stored_role old_failed_attempts <<< "$user_line"
        # Add updated user line
        echo "${stored_username}:${stored_hash}:${stored_role}:${new_failed_attempts}" >> "$temp_file"
    fi
...
generate_report() {
    ...
    
    {
        ...
        if [[ -f "$USERS_DB" ]]; then
            ...
            echo ""
            echo "User Details:"
            while IFS=':' read -r username hash role failed; do
