...
# Goal 5.1 Implementation - Security and Access Control System (Working)
# Priority: High
...
authenticate_user() {
    ...
    if [[ "$input_hash" == "$stored_hash_only" ]]; then
        # Successful login
        sed -i "s/^${username}:.*$/${username}:${stored_hash}:${stored_role}:0/" "$USERS_DB" 2>/dev/null || true
        ...
        sed -i "s/^${username}:.*$/${username}:${stored_hash}:${stored_role}:${new_failed_attempts}/" "$USERS_DB" 2>/dev/null || true
