...
func (bw *BinaryWriter) WriteString(value string) error {
	// Write length first
	err := bw.WriteUint32(uint32(len(value)))
	if err != nil {
		return err
	}
...
// BinaryFormat represents the binary format version
...
func (bf *BinaryFormat) WriteHeader(writer io.Writer) error {
	...
	// Write version
	err = bw.WriteUint32(bf.Version)
	if err != nil {
		return err
	}
	
	// Write header size
	err = bw.WriteUint32(bf.HeaderSize)
	if err != nil {
		return err
	}
...
func (bf *BinaryFormat) ReadHeader(reader io.Reader) error {
	...
	// Verify magic
	if magic[0] != 'T' || magic[1] != 'U' || magic[2] != 'S' || magic[3] != 'K' {
		return fmt.Errorf("invalid magic bytes: %v", magic)
	}
