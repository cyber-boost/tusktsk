...
{
    /// <summary>
    /// MySQL Operator for TuskLang C# SDK
    /// 
    /// Provides MySQL database operations with support for:
    ...
    /// // Execute query
    /// var result = @mysql({
    ///   connection: "Server=localhost;Database=testdb;Uid=user;Pwd=pass",
    ...
    /// ```
    /// </summary>
    public class MySqlOperator : BaseOperator
    {
        ...
        public override string GetName() => "mysql";
        
        protected override string GetDescription() => "MySQL database operator for executing queries and managing connections";
        
        protected override Dictionary<string, string> GetExamples()
        {
            return new Dictionary<string, string>
            {
                ["query"] = "@mysql({connection: connString, query: \"SELECT * FROM users\"})",
                ["insert"] = "@mysql({connection: connString, query: \"INSERT INTO users (name) VALUES (@name)\", parameters: {name: \"John\"}})",
                ["transaction"] = "@mysql({connection: connString, query: \"UPDATE accounts SET balance = balance - 100\", transaction: true})",
                ["procedure"] = "@mysql({connection: connString, query: \"CALL update_user_status(@user_id, @status)\", parameters: {user_id: 1, status: \"active\"}})"
            };
        }
        ...
        protected override async Task<object> ExecuteOperatorAsync(Dictionary<string, object> config, Dictionary<string, object> context)
        {
            ...
            var sslMode = GetContextValue<string>(config, "ssl_mode", "Preferred");
            ...
            {
                Log("error", "MySQL operation failed", new Dictionary<string, object>
                {
            ...
            string isolationLevel, int commandTimeout, int connectionTimeout, string sslMode, int timeout)
        {
            ...
            // In a real implementation, you would use MySqlConnector or another MySQL client
            ...
            try
            {
                ...
                Log("info", "MySQL operation completed", new Dictionary<string, object>
                {
        ...
        private string MaskConnectionString(string connectionString)
        {
            ...
            var passwordIndex = masked.IndexOf("Pwd=", StringComparison.OrdinalIgnoreCase);
            if (passwordIndex >= 0)
            {
                var startIndex = passwordIndex + 4;
