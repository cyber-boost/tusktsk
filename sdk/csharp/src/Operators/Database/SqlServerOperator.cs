...
{
    /// <summary>
    /// SQL Server Operator for TuskLang C# SDK
    /// 
    /// Provides SQL Server database operations with support for:
    ...
    /// ```csharp
    /// // Execute query
    /// var result = @sqlserver({
    ///   connection: "Server=localhost;Database=testdb;User Id=user;Password=pass",
    ...
    /// ```
    /// </summary>
    public class SqlServerOperator : BaseOperator
    {
        public SqlServerOperator()
        {
            ...
            { 
                ...
                "pool_size", "command_timeout", "connection_timeout", "trusted_connection",
        ...
        public override string GetName() => "sqlserver";
        
        protected override string GetDescription() => "SQL Server database operator for executing queries and managing connections";
        
        protected override Dictionary<string, string> GetExamples()
        {
            return new Dictionary<string, string>
            {
                ["query"] = "@sqlserver({connection: connString, query: \"SELECT * FROM users\"})",
                ["insert"] = "@sqlserver({connection: connString, query: \"INSERT INTO users (name) VALUES (@name)\", parameters: {name: \"John\"}})",
                ["transaction"] = "@sqlserver({connection: connString, query: \"UPDATE accounts SET balance = balance - 100\", transaction: true})",
                ["procedure"] = "@sqlserver({connection: connString, query: \"EXEC update_user_status @user_id, @status\", parameters: {user_id: 1, status: \"active\"}})"
            };
        }
        ...
        protected override async Task<object> ExecuteOperatorAsync(Dictionary<string, object> config, Dictionary<string, object> context)
        {
            ...
            var trustedConnection = GetContextValue<bool>(config, "trusted_connection", false);
            ...
            try
            {
                ...
                return await ExecuteDatabaseOperationAsync(
                    ...
                    commandTimeout, connectionTimeout, trustedConnection, timeout
                );
            }
            catch (Exception ex)
            {
                Log("error", "SQL Server operation failed", new Dictionary<string, object>
                {
        ...
        private async Task<object> ExecuteDatabaseOperationAsync(
            ...
            string isolationLevel, int commandTimeout, int connectionTimeout, bool trustedConnection, int timeout)
        {
            // This is a simplified implementation
            // In a real implementation, you would use System.Data.SqlClient or Microsoft.Data.SqlClient
            ...
            try
            {
                ...
                else if (queryLower.StartsWith("exec") || queryLower.StartsWith("execute"))
                {
                ...
                Log("info", "SQL Server operation completed", new Dictionary<string, object>
                {
