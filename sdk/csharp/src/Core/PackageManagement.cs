...
";
        ...
        public async Task<string> GeneratePackageJsonAsync(NPMConfig config)
        {
            var packageJson = new
            {
                ...
                description = config.Description,
                main = config.Main,
                dependencies = config.Dependencies.ToDictionary(d => d, d => "*"),
                devDependencies = config.DevDependencies.ToDictionary(d => d, d => "*"),
                scripts = config.Scripts
            };

            return await Task.FromResult(System.Text.Json.JsonSerializer.Serialize(packageJson, new System.Text.Json.JsonSerializerOptions { WriteIndented = true }));
        }

        public async Task<string> GenerateNodeJSIndexFileAsync()
        {
            return await Task.FromResult(@"const express = require('express');
const app = express();
...
app.get('/', (req, res) => {
    res.json({ message: 'TuskLang Node.js Application Running!' });
});
