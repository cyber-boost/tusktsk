...
namespace TuskLang.Core.Connection
{
    public class MySqlConnectionPool : IConnectionPool
    {
        ...
        public MySqlConnectionPool(string connectionString, ConnectionPoolOptions options, ILogger logger)
        {
        ...
        public async Task<IDbConnection> GetConnectionAsync(CancellationToken cancellationToken = default)
        {
            ...
            _logger?.LogDebug("Getting MySQL connection from pool");
            await Task.Delay(100, cancellationToken);
            return new MySqlConnectionPlaceholder();
        }

        public void ReturnConnection(IDbConnection connection)
        {
            if (_disposed) return;
            _logger?.LogDebug("Returning MySQL connection to pool");
        }
        ...
        {
            if (_disposed) return;
            _logger?.LogInformation("Recovering MySQL connection pool");
        ...
        public void Dispose()
        {
            ...
            {
                _disposed = true;
                _logger?.LogDebug("MySQL connection pool disposed");
            }
        }

        private class MySqlConnectionPlaceholder : IDbConnection
        {
            public string ConnectionString { get; set; } = "";
            public int ConnectionTimeout { get; set; } = 30;
            public string Database { get; set; } = "";
            public ConnectionState State { get; set; } = ConnectionState.Open;
            public IDbTransaction BeginTransaction() => new MySqlTransactionPlaceholder();
            public IDbTransaction BeginTransaction(IsolationLevel il) => new MySqlTransactionPlaceholder();
            public void ChangeDatabase(string databaseName) { }
            public void Close() { }
            public IDbCommand CreateCommand() => new MySqlCommandPlaceholder();
            public void Open() { }
            public void Dispose() { }
        }

        private class MySqlTransactionPlaceholder : IDbTransaction
        {
        ...
        private class MySqlCommandPlaceholder : IDbCommand
        {
