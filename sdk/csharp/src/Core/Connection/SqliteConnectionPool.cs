...
namespace TuskLang.Core.Connection
{
    public class SqliteConnectionPool : IConnectionPool
    {
        ...
        public SqliteConnectionPool(string connectionString, ConnectionPoolOptions options, ILogger logger)
        {
        ...
        public async Task<IDbConnection> GetConnectionAsync(CancellationToken cancellationToken = default)
        {
            ...
            _logger?.LogDebug("Getting SQLite connection from pool");
            await Task.Delay(100, cancellationToken);
            return new SqliteConnectionPlaceholder();
        }

        public void ReturnConnection(IDbConnection connection)
        {
            if (_disposed) return;
            _logger?.LogDebug("Returning SQLite connection to pool");
        }
        ...
        {
            if (_disposed) return;
            _logger?.LogInformation("Recovering SQLite connection pool");
        ...
        public void Dispose()
        {
            ...
            {
                _disposed = true;
                _logger?.LogDebug("SQLite connection pool disposed");
            }
        }

        private class SqliteConnectionPlaceholder : IDbConnection
        {
            public string ConnectionString { get; set; } = "";
            public int ConnectionTimeout { get; set; } = 30;
            public string Database { get; set; } = "";
            public ConnectionState State { get; set; } = ConnectionState.Open;
            public IDbTransaction BeginTransaction() => new SqliteTransactionPlaceholder();
            public IDbTransaction BeginTransaction(IsolationLevel il) => new SqliteTransactionPlaceholder();
            public void ChangeDatabase(string databaseName) { }
            public void Close() { }
            public IDbCommand CreateCommand() => new SqliteCommandPlaceholder();
            public void Open() { }
            public void Dispose() { }
        }

        private class SqliteTransactionPlaceholder : IDbTransaction
        {
        ...
        private class SqliteCommandPlaceholder : IDbCommand
        {
