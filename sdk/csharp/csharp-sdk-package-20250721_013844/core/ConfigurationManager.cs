...
namespace TuskLang.Configuration
{
    ...
    public class ConfigurationManager : IDisposable
    {
        ...
        /// </summary>
        public ConfigurationManager(ConfigurationManagerOptions options = null)
        {
            _options = options ?? new ConfigurationManagerOptions();
        ...
        public async Task<IConfiguration> LoadConfigurationAsync(string filePath, string environment = null)
        {
            ...
            try
            {
                ...
                var configuration = new Configuration(result.Configuration, fullPath, environment, result);
        ...
        public async Task<IConfiguration> LoadConfigurationAsync(string source, string name, string environment = null)
        {
            ...
            try
            {
                ...
                var configuration = new Configuration(result.Configuration, name, environment, result);
        ...
        /// </summary>
        public async Task<ConfigurationValidationResult> ValidateConfigurationAsync(string filePath)
        {
            if (!File.Exists(filePath))
            {
            ...
            try
            {
                var result = await _engine.ProcessFileAsync(filePath);
        ...
        private bool TryGetCachedConfiguration(string cacheKey, out IConfiguration configuration)
        {
            ...
            if (_options.EnableHotReloading && cached.Configuration is Configuration config)
            {
                var filePath = config.SourceFile;
                if (File.Exists(filePath))
                {
                    var lastWrite = File.GetLastWriteTimeUtc(filePath);
                    if (lastWrite > cached.CreatedAt)
                    {
                        _configurationCache.TryRemove(cacheKey, out _);
                        return false;
                    }
        ...
        private IConfiguration MergeConfigurations(List<IConfiguration> configurations)
        {
            ...
            {
                if (config is Configuration tuskConfig)
                {
                    MergeConfiguration(merged, tuskConfig.GetAllSettings());
                    sourceFiles.Add(tuskConfig.SourceFile);
                }
            }
            
            return new Configuration(merged, string.Join(";", sourceFiles), null, null);
        }
