...
public class PerformanceLogger
{
    ...
    public PerformanceLogger(ILogger logger, TuskLang config)
    {
        _logger = logger;
        _config = config;
    ...
    {
        return new OperationTimer(_logger, operationName, _config);
    }
    ...
    public async Task<T> MeasureAsync<T>(string operationName, Func<Task<T>> operation)
    {
        ...
        try
        {
            ...
            var threshold = _config.GetValue<int>("performance.slowOperationThresholdMs", 1000);
            var level = stopwatch.ElapsedMilliseconds > threshold ? LogLevel.Warning : LogLevel.Information;
            
            _logger.Log(level, "Operation {Operation} completed in {Duration}ms", 
    ...
    private class OperationTimer : IDisposable
    {
        ...
        public OperationTimer(ILogger logger, string operationName, TuskLang config)
        {
            _logger = logger;
        ...
        public void Dispose()
        {
            ...
            var threshold = _config.GetValue<int>("performance.slowOperationThresholdMs", 1000);
            var level = _stopwatch.ElapsedMilliseconds > threshold ? LogLevel.Warning : LogLevel.Information;
...
public class MemoryLogger
{
    ...
    public MemoryLogger(ILogger logger, TuskLang config)
    {
        _logger = logger;
        _config = config;
        
        var intervalMinutes = config.GetValue<int>("logging.memoryLogIntervalMinutes", 5);
        _timer = new Timer(LogMemoryUsage, null, TimeSpan.Zero, TimeSpan.FromMinutes(intervalMinutes));
    }
    ...
    private void LogMemoryUsage(object state)
    {
        ...
        var threshold = _config.GetValue<int>("logging.memoryWarningThresholdMB", 1000);
        var level = memoryMB > threshold ? LogLevel.Warning : LogLevel.Information;

        _logger.Log(level, "Current memory usage: {MemoryMB}MB", memoryMB);
    }
